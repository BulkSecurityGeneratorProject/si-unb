...
export class ReservaService {
  ...
  protected convertDateFromClient(reserva: IReserva): IReserva {
    const copy: IReserva = Object.assign({}, reserva, {
      horaReserva: reserva.horaReserva != null && reserva.horaReserva.isValid() ? reserva.horaReserva.toJSON() : null
    });
    return copy;
  }
  ...
  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.horaReserva = res.body.horaReserva != null ? moment(res.body.horaReserva) : null;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((reserva: IReserva) => {
        reserva.horaReserva = reserva.horaReserva != null ? moment(reserva.horaReserva) : null;
      });
    }
    return res;
  }
